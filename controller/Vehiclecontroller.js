...
exports.createVehicleModel = async (req, res) => {
  try {
    ...
    try {
      await cacheDel("vehicle-models:all");
      console.log("[CACHE] Đã xóa cache vehicle-models:all");
    } catch (err) {
      console.error("[CACHE] Lỗi khi xóa cache:", err);
    }
...
exports.createVehicle = async (req, res) => {
  try {
    ...
    // Invalidate cache (lần đọc tiếp theo sẽ query DB và cache lại)
    await cacheDel(`vehicles:user:${user_id}`);
    await cacheDelByPattern("vehicles:all");
...
exports.getVehicleModels = async (req, res) => {
  try {
    ...
    await cacheSet(key, models, 600); // Cache 10 phút
...
exports.getUserVehicle = async (req, res) => {
  try {
    ...
    const key = `vehicles:user:${user_id}`;
    ...
    await cacheSet(key, vehicles, 300); // Cache 5 phút
